...
function CreateUserScreen(props) {
  ...
  return(
    <div className="container">
      ...
      <h1>Login to N Kalari</h1>

        <form onSubmit={handleSubmit}>
          <div className="form-group">
            <small>Mobile</small>
            <input onChange={e => setUsername(e.target.value)} name="mobile" placeholder="" type="text"/>
            <small class="error">{usernameError}</small>
          </div>
          <div className="form-group">
            <label htmlFor="email-register">
              <small>Email</small>
              <input onChange={e => setEmail(e.target.value)} name="email" placeholder="(Optional)" />
